...
| Catalogue revisions applied (TASK-0022: plans stop promising PITR, a connection count, a dedicated IP/DB; `1h` backups become `hourly`) | `php artisan onhost:catalog:revise` (dry run), then `--apply`; docs/runbooks/pricing.md *Catalogue revisions* | `onhost:doctor` → *every catalogue revision is applied* OK |
