...
| Změna tarifu | cart line `config.upgrade_of` ; `GET /v1/services/{id}/plans` | `PlanFit` decides whether the service fits the target plan (sites, the space its further sites hold, the test copy, the PHP version its sites run) — refused in the quote and again when the paid line is applied, with every reason at once; the offer marks each plan `fits` / `blockers`. What the panel owns (databases, mailboxes, FTP) is deliberately not checked here: `ServiceFeatures` counts those at the panel before every new one. On ISPConfig the change also moves the CLIENT's limits (`client_update`, read-merge-write, never the password), because the panel checks a site against them; the site itself gets `site_nvme_gb` — what the plan leaves after the other sites |
| Weby tarifu | `site.create` (domain, nvme_gb, php_version), `site.delete` (site_id) ; resource `sites` | `ServiceSites` + `SiteWorkflow`: a further site of the plan is a site of its own (own document root, PHP version, certificate, databases, FTP, backups, panel), provisioned by the ordinary website saga as a carried service and removed by ITS OWN cancellation (HIGH + step-up, so it is archived first). The plan's space is divided between the sites — ISPConfig sums a client's `hd_quota` against `limit_web_quota`, so the new site takes what is free and the rest comes out of the main site in the same run; a site is never left with less than it already stores (+15 %, read from the node). The `subdomains` limit means extra NAMES on one site again (`aliases` in the plan, or none) |
| Weby, které služba nese | — (lifecycle) | A web hosting service holds further sites of the same customer as services of their own (`tags.billing = included`, `tags.parent_service_id`): the test copy today, the plan's further sites next. `IncludedServices` is the ONE place that decides what belongs to a service (always scoped to the organisation). The cancellation and the removal send every carried site into **its own** cancellation — so each gets its own final archive before anything is deleted — and drop the staging link; the suspension and the resume reach them too, and the platform lifts **only the hold it imposed** (a site the customer stopped, or one an abuse case holds, stays down). A site that never finished provisioning is simply closed: there is nothing at the panel to remove |
...
| Archive download | `POST /v1/services/archives/{id}/download` | the zip carries `SHA256SUMS` (the format `sha256sum -c` reads) and a `README.txt`: what each file is, a stock command to restore it, and what is NOT in the archive (H28). Packages built before that are rebuilt on the next download (`download.format`). A VPS archive holds the metadata only — the disk image stays a protected snapshot on the backup server. |
