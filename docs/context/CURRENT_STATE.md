...
- The Brain's requirement cards drive the hardening now: a changed panel address locks the instance until a probe
  ...
  it starts; an emergency, late-approved or back-dated window counts as downtime, and the status page says which (H15). Event payloads keep who wrote a message (`author_*`): the outbox masked
...
- Pest: 473 tests, 11 124 assertions green; Pint clean; Larastan level 5 clean (the baseline holds the older typing
