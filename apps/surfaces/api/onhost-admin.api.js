...
(function () {
  ...
  function capacityTable(cmp, _) {
    ...
    var bf = c.budget_forecast; // §5r-5: next month's node purchases against the cap
    if (bf && bf.nodes) rows.unshift([tr(cmp, 'Rozpočet ', 'Budget ') + bf.month, bf.nodes + tr(cmp, ' uzlů podle trendu · odhad ', ' nodes by the trend · estimate ') + fmtMoney({ minor: bf.total_minor, currency: bf.currency }) + (bf.budget_minor ? tr(cmp, ' · limit ', ' · cap ') + fmtMoney({ minor: bf.budget_minor, currency: bf.currency }) : tr(cmp, ' · bez limitu', ' · no cap')) + (bf.unpriced ? tr(cmp, ' · ' + bf.unpriced + ' fondů bez ceny', ' · ' + bf.unpriced + ' pools unpriced') : ''), [String(bf.nodes), tr(cmp, 'uzlů', 'nodes')], bf.over ? 'hot' : 'ok', bf.over ? tr(cmp, 'nad limitem', 'over the cap') : tr(cmp, 'v limitu', 'within the cap'), []]);
