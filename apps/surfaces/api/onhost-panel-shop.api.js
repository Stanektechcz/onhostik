...
(function () {
  ...
  function searchDomains() {
    ...
    window.OnhostApi.post('/domains/check', { names: names, currency: 'CZK' }).then(function (r) {
      set({ dbusy: false, dresults: (r.data || r || []).map(function (x) { return { name: x.fqdn || x.name || x.domain, available: x.available === true, note: x.reason || '' }; }) });
