...
(function () {
  ...
  function orderForm() {
    ...
    var money = !!(state.org && state.org.can && state.org.can.move_money); // spending the customer's credit or invoicing them is finance's call (H348)
    if (!money) f.o_pay = 'bank';
    html += '<label>Platba<select data-f="o_pay"><option value="bank"' + (f.o_pay !== 'wallet' && f.o_pay !== 'postpaid' ? ' selected' : '') + '>Převodem (zálohová faktura)</option>' + (money ? '<option value="wallet"' + (f.o_pay === 'wallet' ? ' selected' : '') + '>Z kreditu zákazníka</option><option value="postpaid"' + (f.o_pay === 'postpaid' ? ' selected' : '') + '>Na fakturu (schválený úvěr)</option>' : '') + '</select>' + (money ? '' : '<span class="ohac-muted">Platbu z kreditu nebo na fakturu zadává fakturační oddělení; zákazník tuto objednávku uhradí převodem sám.</span>') + '</label>' +
