...
it('lists every ONhost variable the configuration reads and carries no secret values', function () {
    ...
    $missing = array_values(array_diff(array_unique($keys[1]), array_keys($have), ['ONHOST_DEMO_MODE', 'ONHOST_OUTBOX_EAGER', 'ONHOST_DEFAULT_LOCALE', 'ONHOST_PORTAL_URL', 'ONHOST_PLATFORM_BACKUP_FILES_ROOT', 'ONHOST_ORDER_RISK_DISPOSABLE', 'ONHOST_ORDER_RISK_FIRST_CZK', 'ONHOST_ORDER_RISK_FIRST_EUR', 'WEDOS_ENDPOINT', 'WEDOS_PUBLIC_PRICELIST_URL', 'WEDOS_CLOCK_MAX_OFFSET_SECONDS', 'COMGATE_BASE_URL', 'COMGATE_SECRET_REF', 'GOPAY_BASE_URL', 'PEPPOL_BASE_URL', 'AI_OPENAI_EMBEDDING_MODEL', 'AI_ANTHROPIC_BASE_URL']));
    expect($missing)->toBe([]);
