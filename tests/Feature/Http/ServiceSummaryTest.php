...
it('summarises plan, billing, project, backup, monitor and operations of a service for the detail, the list and the panel rows', function () {
    ...
    Backup::query()->create(['service_id' => $service->id, 'organization_id' => $org->id, 'provider_instance_id' => $service->provider_instance_id, 'kind' => 'scheduled', 'state' => 'completed', 'size_bytes' => 1024, 'started_at' => now()->subHours(7), 'finished_at' => now()->subHours(6), 'verified_at' => now()->subHours(5), 'offsite' => true, 'protected' => false]);
