            ...
            <table class="table table-hover table-sm mb-0">
                <thead class="table-dark">
                    <tr>
                        ...
                        <th>Název</th>
                        <th>Slug</th>
                        <th class="text-end">Cena/měs.</th>
                        <th class="text-end">Aktivací</th>
                ...
                <tbody>
                    @forelse($addons as $addon)
                        <tr>
                            ...
                            <td class="small fw-semibold">{{ $addon->name }}</td>
                            <td class="small font-monospace text-muted">{{ $addon->slug }}</td>
                            <td class="text-end small">{{ number_format($addon->price_czk / 100, 2) }} Kč</td>
                            <td class="text-end small">{{ $addon->subscriptions_count }}</td>
                            ...
                            <td class="text-end">
                                ...
                                <form action="{{ route('admin.service-addons.destroy', $addon) }}" method="POST" class="d-inline"
