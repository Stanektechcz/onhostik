        ...
        <div class="col-span-12 md:col-span-4">
            <div class="card text-center h-full">
                <div class="card-body py-3">
                    ...
                    <div class="h3 font-bold mb-0 text-primary">{{ number_format($total) }}</div>
        ...
        <div class="col-span-12 md:col-span-4">
            <div class="card text-center h-full">
                <div class="card-body py-3">
                    ...
                    <div class="h3 font-bold mb-0 {{ $errorRate > 10 ? 'text-danger' : ($errorRate > 5 ? 'text-warning' : 'text-success') }}">
                        {{ $errorRate }}%
                    </div>
        ...
        <div class="col-span-12 md:col-span-4">
            <div class="card text-center h-full">
                <div class="card-body py-3">
                    ...
                    <div class="h3 font-bold mb-0 {{ $avgResponse > 500 ? 'text-danger' : ($avgResponse > 200 ? 'text-warning' : 'text-success') }}">
                        ...
                        <tbody>
                            @forelse($topEndpoints as $ep)
                                <tr>
                                    ...
                                    <td class="text-right small">{{ number_format($ep->hits) }}</td>
                                    <td class="text-right small">{{ $ep->avg_ms }}</td>
                                </tr>
                            @empty
                ...
                <div class="card-body">
                    ...
                    @forelse($statusDist as $row)
                        ...
                        <div class="flex items-center mb-2 gap-2">
                            ...
                            <div class="progress grow" style="height:14px">
                        ...
                        <tbody>
                            @forelse($trend as $row)
                                <tr>
                                    ...
                                    <td class="text-right small">{{ $row->hits }}</td>
                                    <td class="text-right small {{ $row->errors > 0 ? 'text-danger' : 'text-muted' }}">{{ $row->errors }}</td>
                                </tr>
                            @empty
                        ...
                        <tbody>
                            @forelse($topUsers as $u)
                                <tr>
                                    ...
                                    <td class="text-right small">{{ number_format($u->hits) }}</td>
                                    <td class="text-right small {{ $u->errors > 0 ? 'text-danger' : 'text-muted' }}">{{ $u->errors }}</td>
                                </tr>
                            @empty
                                <tr><td colspan="3" class="text-center text-muted">Žádná data</td></tr>
                            @endforelse
                ...
                <tbody>
                    @forelse($recent as $log)
                        <tr>
                            ...
                            <td class="text-right small">{{ $log->response_time_ms ?? '—' }}</td>
