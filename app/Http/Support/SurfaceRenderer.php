        ...
        if ($surface === 'panel' && ! $demo) {
            ...
            $html = str_replace(
                ...
                [
                    ...
                    "[_('Zákazník', 'Customer'), ({$bill} && window.ONHOST_PANEL.billing && window.ONHOST_PANEL.billing.organization.name) || 'Skladomat s.r.o.'],",
                    "[_('Tarif', 'Plan'), {$bill} ? (window.ONHOST_PANEL.billing.organization.billing_mode === 'postpaid' ? _('Firemní, na fakturu', 'Business, invoiced') : _('Předplacený kredit', 'Prepaid credit')) : _('Firemní, měsíčně', 'Business, monthly')],",
                    "[_('Nejbližší faktura', 'Next invoice'), {$bill} ? ((window.ONHOST_PANEL.billing.document && /^(due|overdue)$/.test(window.ONHOST_PANEL.billing.document.state)) ? (window.ONHOST_PANEL.billing.document.due || '') + ' · ' + this.money(window.ONHOST_PANEL.billing.document.outstanding) : _('nic k úhradě', 'nothing due')) : _('1. 9. · ', '1 Sep · ') + this.money(totalSpend)],",
                    "[_('Splatnost', 'Payment terms'), {$bill} ? (window.ONHOST_PANEL.billing.terms.due_days + _(' dní · kredit, karta nebo převod', ' days · credit, card or transfer')) : _('30 dní, převodem', '30 days, transfer')],",
                    "[_('Dvoufázové ověření', 'Two-factor'), ({$bill} && window.ONHOST && window.ONHOST.user ? !!window.ONHOST.user.mfa : s.twofa) ? _('zapnuto', 'on') : _('vypnuto', 'off')],\n          [_('SLA', 'SLA'), {$bill} ? ((window.ONHOST_PANEL.billing.sla || []).join(', ') || 'standard') + _(' · kredity automaticky', ' · credits automatic') : _('99,99 %, kredity automaticky', '99.99%, credits automatic')]",
                    "wide: {$real} ? null : {\n        title: _('Poslední deploye', 'Recent deploys'),",
    ...
    public static function cartSeams(string $html): string
    {
        $html = str_replace(
            ...
            [
                ...
                "view !== 'register' ? '' : v.length < 12 ? (cs ? 'Heslo musí mít alespoň 12 znaků.' : 'Use at least 12 characters.') : !/[0-9]/.test(v) ? (cs ? 'Přidejte alespoň jednu číslici.' : 'Add at least one digit.') : !/[a-zA-Z]/.test(v) ? (cs ? 'Přidejte alespoň jedno písmeno.' : 'Add at least one letter.') : ''),",
