...
final class NotificationRouter
{
    ...
    public function handle(OutboxEventDispatched $event): void
    {
        ...
        match ($m->name) {
            ...
            'subscription.renewal_failed' => $this->customer($m, 'dunning', 'Prodloužení služby se nezdařilo', 'Nedostatek kreditu · potřeba '.$money($p['required'] ?? null), '/panel/fakturace', 'hot', $email, 'renewal-failed', ['castka' => $money($p['required'] ?? null), 'do' => substr((string) ($p['period_end'] ?? ''), 0, 10), 'url' => "{$portal}/panel/fakturace"]),
            ...
            'domain.renewal_failed', 'domain.renewal_payment_failed', 'domain.renewal_abandoned' => $this->both($m, 'domain.expiry', "Prodloužení {$p['fqdn']} selhalo", (string) ($p['reason'] ?? ($p['error'] ?? '')), "Prodloužení domény {$p['fqdn']} se nezdařilo", 'Dobijte kredit nebo prodlužte ručně, doména jinak expiruje.', '/sprava/fakturace', '/panel/domeny', 'hot', $email, 'domain-renewal-failed', ['domena' => $p['fqdn'], 'duvod' => (string) ($p['reason'] ?? ($p['error'] ?? '')), 'url' => "{$portal}/panel/domeny"]),
            ...
            'security.password_changed' => $this->user($m, 'security.mfa', 'Heslo bylo změněno', ($p['ip'] ?? ''), '/panel/nastaveni', 'warn', 'security-password', ['ip' => $p['ip'] ?? '', 'url' => "{$portal}/panel/nastaveni"]),
            ...
            'files.infected' => $this->internal($m, 'security', 'Zachycen infikovaný soubor: '.($p['name'] ?? ''), ($p['signature'] ?? 'neznámý podpis').' · '.($p['subject'] ?? '').' '.$m->aggregate_id.' · soubor smazán', '/sprava#/security', 'hot'), // §5r-4
