...
final class MetricRegistry
{
    ...
    public const REGISTRY = [
        ...
        'nvme_gb' => [
            ...
            'sources' => ['web/managed' => 'UsageWatch::measure() disk metric (the site files) against panel quotas.disk_used_bytes/disk_limit_bytes, falling back to nvme_gb; the plan total files + databases + mail (WebDiskTotal, tags.usage.disk_total) is shown to everybody and counts against nvme_gb only from onhost.metering.web_disk_total.enforce_from for services noticed in time', 'cloud/data' => 'ServiceService dispatches ProvisionVpsWorkflow by executor=proxmox, not family, so the managed-database product (family data, executor proxmox) sizes its disk from nvme_gb exactly like a VPS/VDS: ProxmoxComputeProvider disk sized at provision/resize and drift-checked (disk_gb) — usage() has no used-bytes field, so no running percentage', 'game' => 'ProvisionGameServerWorkflow sizes the Pterodactyl allocation\'s disk_mb from nvme_gb'],
