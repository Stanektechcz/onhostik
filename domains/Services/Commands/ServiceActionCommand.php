...
final class ServiceActionCommand extends OrganizationCommand implements RiskAwareCommand
{
    ...
    {
        return in_array((string) $this->get('action'), ['terminate', 'purge', 'restore', 'rollback_snapshot', 'resize', 'reinstall', 'panel.password', 'access.reset', 'rescue.start'], true) ? PermissionCatalog::HIGH : PermissionCatalog::NORMAL;
    }

    public function requiresStepUp(): bool
    {
        return in_array((string) $this->get('action'), ['terminate', 'purge', 'restore', 'rollback_snapshot', 'reinstall', 'panel.password', 'access.reset', 'rescue.start'], true); // a reinstall wipes the server, the panel password opens every server of the account, new keys open the server itself
    }
